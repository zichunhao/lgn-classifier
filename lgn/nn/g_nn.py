...
class MixReps(CGModule):
    ...
    def __init__(self, tau_in, tau_out, real=False, weight_init='randn', gain=1, device=None, dtype=torch.float64):
        ...
        if self.weight_init == 'randn':
            ...
            raise NotImplementedError("weight_init can only be 'randn' or 'rand'; \
                                      other choices are not implemented yet!")
    ...
    def forward(self, reps):
        ...
        if not GTau.from_rep(reps) == self.tau_in:
            raise ValueError(f'Tau of input reps, {GTau.from_rep(reps)}, does not match \
                             initialized tau, {self.tau_in}!')
...
class CatReps(Module):
    ...
    def forward(self, reps):
        ...
        if reps_taus_in != self.taus_in:
            raise ValueError(f'Taus of input reps {reps_taus_in} do not match \
                             the predefined taus_in {self.taus_in}!')
