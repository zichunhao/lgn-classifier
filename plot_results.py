...
def plot_confusion_matrix(args, confusion_matrix, epoch, outpath, is_train):
    ...
    if is_train:
        ...
        plt.savefig(f'{PATH}/confusion_matrix_train_epoch_{epoch+1}.png', dpi=600, transparent=True)
    else:
        plt.savefig(f'{PATH}/confusion_matrix_valid_epoch_{epoch+1}.{args.fig_format}')
        plt.savefig(f'{PATH}/confusion_matrix_valid_epoch_{epoch+1}.png', dpi=600, transparent=True)
...
def plot_roc_curve(args, predictions_onehot, targets_onehot, epoch, outpath, is_train):
    ...
    if is_train:
        ...
        plt.savefig(f'{PATH}/ROC_train_epoch_{epoch+1}.png', dpi=600, bbox_inches='tight', transparent=True)
    else:
        plt.savefig(f'{PATH}/ROC_valid_epoch_{epoch+1}.{args.fig_format}', bbox_inches='tight')
        plt.savefig(f'{PATH}/ROC_valid_epoch_{epoch+1}.png', dpi=600, bbox_inches='tight', transparent=True)
